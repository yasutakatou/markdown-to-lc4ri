...
コマンド行末につくインラインコメント (`command # 説明`) も同様に扱います。コマンド部分だけを `- ` 付きのセルとして残し、コメント部分は削除するか直前の地の文に移します。
...
echo -n 'webpass'  | base64   # d2VicGFzcw==
```
````
...
`cat > ファイルパス << 'EOF' ... EOF` (ファイル書き出し専用のヒアドキュメント) も同じく `- write:` に変換します。

**変換前:**

````markdown
```bash
cat > ~/k8s-tutorial/00-namespace.yaml << 'EOF'
apiVersion: v1
kind: Namespace
metadata:
  name: tutorial
...
```bash
cat ~/k8s-tutorial/00-namespace.yaml
```
````
...
**パス形式の統一**: Markdownでは `~/プロジェクト名/ファイル名` のようにホームディレクトリからの絶対パスを使うことが多いですが、LC4RIでは `cd` で作業ディレクトリに移動した後、**`./ファイル名` のカレントディレクトリ相対パス** に統一します。これにより特定のユーザー環境に依存しない再現可能な記述になります。

**ディレクトリ構成のフラット化**: Markdownでは `nginx/index.html` のようにサブディレクトリにファイルを分けることがありますが、LC4RIでは `cd` 後の作業ディレクトリに **フラットに配置する** のが基本方針です。`- write:` ディレクティブのパスもフラットにします。

**確認コマンドの統一**: Markdownでは `cat ファイルパス` でファイル内容を確認する場合がありますが、LC4RI では `- write:` 後の確認コマンドは `- ls -la ファイル名` に統一します。ファイル内容の確認は必要な場合のみ追加で `- cat ファイル名` を入れます。
...
`kubectl get pods -w` のような **`-w` (watch) オプション付きコマンド** も、プロセスが終了せず `Ctrl+C` で抜けるまで実行し続けるため `- ! ` を付けます。

`kubectl port-forward ... &` のような **`&` によるバックグラウンド実行コマンド** も同様に `- ! ` を付けます。バックグラウンド実行はターミナルセッションの状態を変えるため、後続のセルに影響することがあります。
...
第一に、ヒアドキュメント (`cat <<EOF ... EOF` や `cat > path << 'EOF' ... EOF`) があれば、`- write: <ファイル名>` ディレクティブと言語タグなしのコードブロックに分離します (ルール5)。ファイルパスは `~/` 始まりを `./` 始まりに変換します。

第二に、各コマンド行を取り出してコードブロックの外に並べ、先頭に `- ` を付けます (ルール2)。バックスラッシュ継続行は1セルにまとめます (ルール3)。コード内のシェルコメント行 (`# ...`) およびインラインコメント (`command # 説明`) はコメント部分を削除します。

第三に、`make run`、`kubebuilder create api`、`kubectl logs -f`、`kubectl get -w`、`command &` のようなブロッキング実行コマンドには `- ! ` を付けます (ルール6)。
...
第四に、**対話コマンドへの `- ! ` の付け忘れ**。`make run` や `kubebuilder create api` をそのまま `- ` で実行すると、Notebookがハングします。「実行が即時に終わるか？」を基準に判定してください。`-w` オプション付きの `kubectl get` や `&` バックグラウンド実行コマンドも同様に `- ! ` が必要です。

第五に、**`cat > path << 'EOF' ... EOF` をそのまま残してしまうミス**。`cat > ファイルパス << 'EOF'` 形式のヒアドキュメントは `- write:` ディレクティブに変換する必要があります。変換後の `- write:` に続くコードブロックには言語タグを付けてはいけません。

第六に、**パスを `~/絶対パス` のままにしてしまうミス**。LC4RIでは `cd` で作業ディレクトリに入った後、`~/` 始まりの絶対パスを `./` 始まりの相対パスに統一します。コマンド内のファイルパスをすべて確認してください。
